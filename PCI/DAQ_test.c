...
//
// Note : Single ADC - Disable Burst - Single write to DAC 
//- only individual write seems to work.
...
int main()
{
  ...
  {  
    if (info.BaseAddressSize[i] > 0)
    {
      printf("Aperture %d  Base 0x%x Length %d Type %s\n", i,
             ...
             (int)PCI_MEM_ADDR(info.CpuBaseAddress[i]) : 
             (int)PCI_IO_ADDR(info.CpuBaseAddress[i]), 
  ...
  {
    //*****************************************************************************
    //Digital Port Functions (Switch & LED)
    //*****************************************************************************
